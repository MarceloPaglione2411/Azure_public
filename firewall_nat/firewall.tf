...
resource "azurerm_firewall_policy" "firewallpolicyrosa" {
  name                = "firewallpolicyrosa"
  resource_group_name = local.resource_group_name
  location            = local.location
}
...
resource "azurerm_firewall" "appfirewall" {
  ...
  firewall_policy_id = azurerm_firewall_policy.firewallpolicyrosa.id
  depends_on = [
    azurerm_firewall_policy.firewallpolicyrosa
  ]
}